...
function Home(p) {
  ...
  return (
    <Column f={1} >
      <ScrollView onMoveShouldSetResponder={() => { setTimeout(() => { setenableScroll(true) }, 200); }} scrollEnabled={(Platform.OS === 'web') ? ((navigator?.userAgent?.match('Mobile') == 'Mobile' && !window.matchMedia('(display-mode:standalone)').matches) ? enableScroll : true) : true} >
        <Column>
