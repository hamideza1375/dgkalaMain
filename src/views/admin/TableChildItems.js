...
function ChildItemTable(p) {
  ...
  return (
    <Table
      ...
      btn1={'#e63'}
      btn1onClick={() => { childItemsTable[0]?._id && p.navigation.navigate('SetOffer', { id: childItemsTable[0]._id }) }}
      btn2={'#e33'}
      btn2onClick={() => { childItemsTable[0]?._id && deleteChildItem(childItemsTable[0]._id) }}
      btn3={'#0c4'}
      btn3onClick={() => { childItemsTable[0]?._id && changeAvailable(childItemsTable[0]._id) }}
      btn4={'#09d'}
      btn4onClick={() => { childItemsTable[0]?._id && p.navigation.navigate('EditChildItem', { id: childItemsTable[0]._id }) }}
      btn3Opacity
      titleClick={() => { childItemsTable[0]?._id && p.navigation.navigate('SingleItem', { id: childItemsTable[0]._id }) }}
      object={p.childItem}
