        ...
        >
          ...
          <Animated.View style={[styles.animatedBorder, getBlur && !yub && { borderWidth: 1.2, borderColor: iterPlt, transform: [{ translateX: fadeAnim }] }, (multiline && !initialHeight) && { height: '101%', minHeight: '101%', marginTop: -3 }]} >
            <Input
              ...
              placeholder={p2 ? p2 : p}
              value={inputState}
              onChangeText={(text) => { setinputState(text); int && clearInterval(int); int = setTimeout(() => {setState(text)}, 1000); }}
              onBlur={() => {setState(inputState); setTimeout(() => { setBlur(true); !yub && fadeOut() }, 200); }}
