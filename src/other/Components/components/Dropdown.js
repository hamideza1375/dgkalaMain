...
const Dropdown = ({ top, value, root, children }) => {
  ...
  return (
    ...
    setclientY(!top ? e.nativeEvent.pageY : e.nativeEvent.pageY - e.nativeEvent.locationY - top); setdropdownValue(value); } }} style={root ? { display: shownDropdown ? "flex" : 'none', position: 'absolute', zIndex: 1000, top: dropdownBottom ? null : clientY, left: dropdownRight ? null : clientX, borderWidth: 1, borderRadius: 3, padding: 5, backgroundColor: '#eee' } : { width: '100%', height: '100%' }} >
      {children}
    </Pressable>
  );
}
